...
public class PlaneRenderer {
    ...
    // Shader names.
    private static final String VERTEX_SHADER_NAME = "arcore-example/plane.vert";
    private static final String FRAGMENT_SHADER_NAME = "arcore-example/plane.frag";

    private static final String VERTEX_SHADER_FILENAME = "shaders/plane.vert";
    private static final String FRAGMENT_SHADER_FILENAME = "shaders/plane.frag";
    ...
    // Shader attributes
    private static final String PLANE_XZ_POSITION_ALPHA = "PLANE_XZ_POSITION_ALPHA";

    // Shader uniforms
    private static final String LINE_COLOR = "LINE_COLOR";
    private static final String DOT_COLOR = "DOT_COLOR";
    private static final String GRID_CONTROL_UNIFORM = "GRID_CONTROL_UNIFORM";
    private static final String PLANE_UV_MATRIX = "PLANE_UV_MATRIX";
    ...
    private GLSLProgram planeProgram;
     ...
     */
    public void init(Context context, String gridDistanceTextureName) {

        String vertexShader = ShaderManager.loadShaderFile(context, VERTEX_SHADER_NAME, VERTEX_SHADER_FILENAME);
        String passthroughShader = ShaderManager.loadShaderFile(context, FRAGMENT_SHADER_NAME, FRAGMENT_SHADER_FILENAME);

        planeProgram = new GLSLProgram("arcore-example/PlaneRenderer", vertexShader, passthroughShader);
        planeProgram.declareAttribute(PLANE_XZ_POSITION_ALPHA, "a_XZPositionAlpha");
        planeProgram.declareUniform(GLSLProgram.M_MATRIX, "u_Model");
        planeProgram.declareUniform(GLSLProgram.NORMAL, "u_Normal");
        planeProgram.declareUniform(GLSLProgram.MVP_MATRIX, "u_ModelViewProjection");
        planeProgram.declareUniform(GLSLProgram.DIFFUSE_TEXTURE, "u_Texture");
        planeProgram.declareUniform(LINE_COLOR, "u_lineColor");
        planeProgram.declareUniform(DOT_COLOR, "u_dotColor");
        planeProgram.declareUniform(GRID_CONTROL_UNIFORM, "u_gridControl");
        planeProgram.declareUniform(PLANE_UV_MATRIX, "u_PlaneUvMatrix");
        ...
        try {
            ...
            GLUtils.checkGLError(TAG, "Texture loading");
        } catch (IOException ex) {
            Log.e(TAG, "Texture loading: " + ex);
        }
    }
    ...
    private void draw(float[] cameraView, float[] cameraPerspective, float[] planeNormal) {
        ...
        GLES20.glVertexAttribPointer(
                planeProgram.getAttributeLocation(PLANE_XZ_POSITION_ALPHA),
                COORDS_PER_VERTEX,
                GLES20.GL_FLOAT,
                false,
        ...
        GLES20.glUniformMatrix4fv(planeProgram.getUniformLocation(GLSLProgram.M_MATRIX), 1,
                false, modelMatrix, 0);

        GLES20.glUniform3f(planeProgram.getUniformLocation(GLSLProgram.NORMAL),
                planeNormal[0], planeNormal[1], planeNormal[2]);

        GLES20.glUniformMatrix4fv(planeProgram.getUniformLocation(GLSLProgram.MVP_MATRIX), 1,
                false, modelViewProjectionMatrix, 0);

        indexBuffer.rewind();
        GLES20.glDrawElements(
                GLES20.GL_TRIANGLE_STRIP, indexBuffer.limit(), GLES20.GL_UNSIGNED_SHORT, indexBuffer);
        GLUtils.checkGLError(TAG, "Drawing plane");
    }
    ...
    public void drawPlanes(Collection<Plane> allPlanes, Pose cameraPose, float[] cameraPerspective) {
        ...
        // Set up the shader.
        if (!planeProgram.isCompiled())
            planeProgram.compile();
        ...
        GLES20.glActiveTexture(GLES20.GL_TEXTURE0);
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textures[0]);
        GLES20.glUniform1i(planeProgram.getUniformLocation(GLSLProgram.DIFFUSE_TEXTURE), 0);

        // Shared fragment uniforms.
        GLES20.glUniform4fv(planeProgram.getUniformLocation(GRID_CONTROL_UNIFORM), 1, GRID_CONTROL, 0);

        // Enable vertex arrays
        GLES20.glEnableVertexAttribArray(planeProgram.getAttributeLocation(PLANE_XZ_POSITION_ALPHA));

        GLUtils.checkGLError(TAG, "Setting up to draw planes");
        ...
        for (SortablePlane sortedPlane : sortedPlanes) {
            ...
            GLES20.glUniform4fv(planeProgram.getUniformLocation(LINE_COLOR), 1, planeColor, 0);
            GLES20.glUniform4fv(planeProgram.getUniformLocation(DOT_COLOR), 1, planeColor, 0);
            ...
            GLES20.glUniformMatrix2fv(planeProgram.getUniformLocation(PLANE_UV_MATRIX), 1,
                    false, planeAngleUvMatrix, 0);

            draw(cameraView, cameraPerspective, normal);
        }

        // Clean up the state we set
        GLES20.glDisableVertexAttribArray(planeProgram.getAttributeLocation(PLANE_XZ_POSITION_ALPHA));
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, 0);
        GLES20.glDisable(GLES20.GL_BLEND);
        GLES20.glDepthMask(true);
        GLES20.glClearColor(0.1f, 0.1f, 0.1f, 1.0f);

        GLUtils.checkGLError(TAG, "Cleaning up after drawing planes");
    }
